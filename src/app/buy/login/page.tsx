...
import { User, ShieldCheck, Loader2, Phone, ArrowLeft, CheckCircle2 } from "lucide-react";
import {
...
export default function BuyerLoginPage() {
  ...
  const handleLineLogin = () => {
    if (isLoggedIn && profile?.userId) {
      // Already logged into LINE, check linking
      setMode("line-phone");
    } else {
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Link LINE userId to this staff
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-6 py-12">
      <div className="w-full max-w-sm space-y-8">
        ...
        {/* === MODE: Choose === */}
        {mode === "choose" && (
          <Card className="space-y-4 border-slate-300 bg-white p-6">
            <Button
        ...
        {mode === "line-linking" && linkSuccess && (
          <Card className="space-y-5 border-emerald-300 bg-white p-6 text-center">
